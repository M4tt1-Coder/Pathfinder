...
fn two_dimensional_node_parses_valid_coordinate_string() {
    let node = TwoDimensionalNode::<i32>::from_str("CityA:12,34")
...
fn two_dimensional_node_requires_colon_separator() {
    let err = TwoDimensionalNode::<i32>::from_str("CityA-12,34")
        .expect_err("colon is required");
...
fn two_dimensional_node_requires_two_coordinates() {
    let err = TwoDimensionalNode::<i32>::from_str("CityA:12")
        .expect_err("two coordinates are required");
...
fn two_dimensional_node_supports_f32_coordinates() {
    let node =
        TwoDimensionalNode::<f32>::new(1.5, -2.25, "FloatNode".to_string())
